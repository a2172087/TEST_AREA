...
def process_autoz_log(file_path):
    ...
    # 從末尾開始向前搜索 "Last Contact found"
    for i in range(len(lines)-1, -1, -1):
        if "Last Contact found" in lines[i]:
            # 直接提取該行的時間戳（格式：2025-05-12 16:03:01.689）
            timestamp_part = lines[i].split(" | ")[0].strip()
            # 將時間戳格式從 YYYY-MM-DD HH:MM:SS.fff 轉換為 YYYY/MM/DD HH:MM:SS.fff
            timestamp_part = timestamp_part.replace("-", "/")
            last_contact_timestamp = timestamp_part
            break
    
    # 只打印最終結果
    if last_contact_timestamp is not None:
        print(f"Found Last Contact timestamp: {last_contact_timestamp}")
    else:
        print("Could not find 'Last Contact found' in log file.")
...
def process_all_txt(file_path, autoz_log_timestamp):
    ...
    # 尋找 Auto Z complete 點（Last Contact found 之後的第一個 ndlp-correct）
    ...
    for line in lines:
        ...
        if match:
            ...
            # Last Contact found 之後的第一個 ndlp-correct 就是 Auto Z complete 點
    ...
    if not found_autoz_complete:
        print("Could not find Auto Z complete point (first ndlp-correct after Last Contact found).")
